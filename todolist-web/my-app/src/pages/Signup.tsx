        ...
        <Paper elevation={10} className={classes.paperStyle}>
          ...
          <Formik
            ...
            onSubmit={async(data, { setSubmitting, resetForm,setErrors }) => {
              setSubmitting(true);
              //make async call
              const response = await dispatch(signUpAction({account:data.account,password:data.password}))
              if(signUpAction.fulfilled.match(response)){
                ...
                if(!response.payload?.msg.includes("帳號")){
          ...
          >
            {({ values, errors, isSubmitting, handleSubmit }) => (
              <Form>
                ...
                <Typography className={classes.accountField}>{t('loginTip')}
                      <Link to="/">
                          {t('login')}
                      </Link>
                </Typography> 
